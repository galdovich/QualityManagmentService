package com.galdovich.qaulity.service.validator;
...
public class ProductValidator {
    ...
    public static boolean validateRequestData(Map<String, String> requestData, List<Customer> customerList, List<Order> orderList) {
        ...
        if (requestData.containsKey(PRODUCT_NAME) != false && !isNameValid(requestData.get(PRODUCT_NAME))) {
            requestData.put(PRODUCT_NAME, EMPTY_VALUE);
            result = false;
        }
        if (requestData.containsKey(PRODUCT_DECIMAL) != false && !isDecimalValid(requestData.get(PRODUCT_DECIMAL))) {
            requestData.put(PRODUCT_DECIMAL, EMPTY_VALUE);
            result = false;
        }
        if (requestData.containsKey(CUSTOMER_NAME) != false && !isCustomerValid(customerList, requestData.get(CUSTOMER_NAME))) {
            requestData.put(CUSTOMER_NAME, EMPTY_VALUE);
            result = false;
        }
        if (requestData.containsKey(ORDER_NAME) != false && !isOrderValid(orderList, requestData.get(ORDER_NAME))) {
            requestData.put(ORDER_NAME, EMPTY_VALUE);
            result = false;
        }
        if (requestData.containsKey(PRODUCT_AMOUNT) != false && !isAmountValid(requestData.get(PRODUCT_AMOUNT))) {
            requestData.put(PRODUCT_AMOUNT, EMPTY_VALUE);
            result = false;
        }
        if (requestData.containsKey(PRODUCT_PRIORITY) != false && !isPriorityValid(requestData.get(PRODUCT_PRIORITY))) {
